...
public class BookService {
    ...
    public int addBooks(Book book) {
        ...
        return jdbcTemplate.update("insert into books(title, author, genre, description, " +
                        "yearPublished, copiesSold, isbn, price) values(?,?,?,?,?,?,?,?)",
                new Object[]{book.getTitle(),book.getAuthor(),book.getGenre(),book.getDescription(),
                book.getYearPublished(), book.getCopiesSold(), book.getIsbn(), book.getPrice()});
     }

     public List<Book> selectAllBooks() {
        return jdbcTemplate.query("Select * from books", (rs,rosnum)->
                new Book(rs.getString("title_string"),
                        rs.getString("author"),
                        rs.getString("genre"),
                        rs.getString("description"),
                        rs.getInt("year published"),
                        rs.getInt("copies sold"),
                        rs.getInt("isbn"),
                        rs.getDouble("price")));
     }
